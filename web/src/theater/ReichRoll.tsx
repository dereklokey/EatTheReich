...
/**
 * The connect / connected-dice strip inside the GoDice panel. Each paired die shows a live
 * status dot (green = connected, grey = dropped its link) and flashes the instant it sends any
 * traffic — so "is it actually connected?" is answerable: wobble the die and watch its tag pulse.
 */
function ConnectRow() {
  const goDice = useGoDice();
        ...
        <>
          ...
          <span className="reichroll__hint">
            {connectedCount > 0
              ...
              : "Link lost — reconnect, or use test dice / tap to set."}
          </span>
        </>
      )}
