...
/** The connect / connected-dice strip inside the GoDice panel. */
function ConnectRow() {
  const goDice = useGoDice();
  const live = goDice.dice.filter((d) => d.connected);
  ...
  return (
    <div className="reichroll__connect">
      {live.length === 0 ? (
        <>
          {goDice.supported ? (
            <button className="reichroll__btn" disabled={goDice.connecting} onClick={() => void goDice.connect()}>
              {goDice.connecting ? "pairing…" : "Connect a die"}
            </button>
          ) : (
        ...
        <div className="reichroll__dice-list">
          {live.map((d) => (
            <span key={d.id} className="reichroll__die-chip" title={d.name}>
              {d.name}
              {d.lastValue != null && <b> · {d.lastValue}</b>}
            </span>
          ))}
          {goDice.supported && (
            <button className="reichroll__link" disabled={goDice.connecting} onClick={() => void goDice.connect()}>
              {goDice.connecting ? "pairing…" : "+ add die"}
            </button>
          )}
        </div>
      )}
